...
def send_command(interface, src, dst, key):
    """
    Tuşa göre doğru cmd_* veya FTP fonksiyonunu çağırır.
    """
    if key == 'T':            # Takeoff
        print("[CMD] TAKEOFF")
        ...
        if not os.path.isfile(path):
            ...
            print("[FTP] Gönderim tamamlandı (START/CHUNK/END tetiklendi).")
...
Tuş atamaları:
  ...
  F → FTP dosya gönderimi (masaüstündeki example.png)
  Q → ÇIKIŞ
"""
    print(instructions)
    while True:
