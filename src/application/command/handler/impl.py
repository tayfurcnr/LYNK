...
def default_handler(cmd_id: int, params: dict | bytes, src_id: int, interface):
    ...
    """
    logger.info(f"[COMMAND] RECV | GOTO BRIDGE | CMD_ID: {cmd_id} from SRC: {src_id}")
    set_last_command(cmd_id, params, params if isinstance(params, dict) else {})
    
    # Send success ACK to notify sender that command was received by the bridge
...
def flight_arming(cmd_id, params, src_id, interface):
    if _is_dict_params(params):
        ...
        logger.info(f"[COMMAND] RECV | CMD: FLIGHT_ARMING | ARM: {arm}, FORCE: {force}")
        set_last_command(cmd_id, params, parsed)
        # send_ack_ok(interface, cmd_id, dst=src_id)
    elif len(params) == 1:
