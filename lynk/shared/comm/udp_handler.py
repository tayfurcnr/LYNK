...
# lynk/tools/comm/udp_handler.py
...
class UDPHandler:
    def __init__(self):
        ...
        self._prio_queue = Queue(maxsize=self.rx_queue_size)
        self._norm_queue = Queue(maxsize=self.rx_queue_size)
        self.running = False
        self.thread = None
    ...
    def _put_drop_oldest(self, q: Queue, data: bytes) -> None:
        try:
            q.put_nowait(data)
        except Full:
            try:
                _ = q.get_nowait()  # eskiyi at
            except Exception:
                pass
            q.put_nowait(data)
    ...
    def _enqueue_latest(self, data: bytes) -> None:
        ...
        frame_type = chr(data[3]) if len(data) > 3 else ""
        if frame_type in ("A", "C", "R"):
            self._put_drop_oldest(self._prio_queue, data)
        else:
            self._put_drop_oldest(self._norm_queue, data)
    ...
    def read(self) -> bytes | None:
        """
        Kuyruktan bir paket döndürür (FIFO). ACK/COMMAND/RESULT önceliklidir.
        """
        try:
            return self._prio_queue.get_nowait()
        except Exception:
            pass
        try:
            return self._norm_queue.get_nowait()
        except Exception:
            return None
