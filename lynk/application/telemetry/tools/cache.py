...
def get_vehicle_telemetry(src_id: int, data_type: str) -> Optional[Dict[str, Any]]:
    """
    Retrieve telemetry data for a specific vehicle and data type.
    Alias for get_device_data with more descriptive name.
    
    Args:
        src_id (int): Vehicle identifier.
        data_type (str): Telemetry category (e.g., "gps", "imu").
    
    Returns:
        dict or None: The telemetry payload including 'timestamp', or None if not found.
    """
    return get_device_data(src_id, data_type)

def get_vehicle_snapshot(src_id: int) -> Optional[Dict[str, Any]]:
    """
    Get complete vehicle data with metadata including vehicle_id.
    
    Args:
        src_id (int): Vehicle identifier.
    
    Returns:
        dict or None: Complete vehicle data with structure:
        {
            "vehicle_id": int,
            "team_id": int | None,
            "last_hop_count": int,
    ...
    with _lock:
        if src_id not in _device_cache:
            return None
        import copy
        data = copy.deepcopy(_device_cache[src_id])
        data["vehicle_id"] = src_id
        return data

def get_cache_snapshot() -> Dict[str, Any]:
    """
    Get entire cache with vehicle list and structured data array.
    Useful for database storage and API responses.
    
    Returns:
        dict: Cache snapshot with structure:
        {
            "vehicles": [1, 2, 3, ...],
            "data": [
                {"vehicle_id": 1, "team_id": 1, "telemetry": {...}},
                ...
            ]
        }
    """
    with _lock:
        import copy
        cache_copy = copy.deepcopy(_device_cache)
        
        return {
            "vehicles": sorted(cache_copy.keys()),
            "data": [
                {"vehicle_id": vid, **vdata}
                for vid, vdata in cache_copy.items()
            ]
        }